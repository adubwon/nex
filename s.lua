...
function Library.CreateWindow(Properties)
    ...
    function Window:Tab(Properties)
        ...
        function Tab:Section(Properties)
            ...
            -- Slider Element - MORE VISIBLE (UPDATED)
            function Section:Slider(Properties)
                ...
                local frame = Library.CreateInstance("Frame", {
                    ...
                    BackgroundTransparency = 1,
                    Size = Library.UDim2(1, 0, 0, IsMobile and 68 or 58), -- Increased height
                    Parent = contentHolder,
                })
                ...
                -- Value label with accent color and bold
                local valueLabel = Library.CreateInstance("TextLabel", {
                    Name = "ValueLabel",
                    Font = Enum.Font.GothamBold, -- Made bold
                    Text = tostring(Slider.Default) .. Slider.Suffix,
                    TextColor3 = CurrentTheme.Accent,
                    TextSize = IsMobile and 16 or 14, -- Slightly larger
                    BackgroundTransparency = 1,
                    Position = Library.UDim2(1, 0, 0, 0),
                    AnchorPoint = Vector2.new(1, 0),
                    Size = Library.UDim2(0, 80, 0, IsMobile and 24 or 20), -- Wider for better visibility
                    Parent = frame,
                })
                ...
                local barBg = Library.CreateInstance("Frame", {
                    ...
                    BackgroundTransparency = 0,
                    BorderSizePixel = 0,
                    Position = Library.UDim2(0, 0, 0, IsMobile and 38 or 34), -- Adjusted position
                    Size = Library.UDim2(1, 0, 0, IsMobile and 14 or 12), -- THICKER (was 8/6)
                    Parent = frame,
                })
                
                Library.CreateInstance("UICorner", {
                    CornerRadius = UDim.new(0, IsMobile and 7 or 6), -- Rounded corners
                    Parent = barBg,
                })
                
                -- Glow effect on slider bar
                local barGlow = Library.CreateInstance("Frame", {
                    Name = "BarGlow",
                    BackgroundColor3 = CurrentTheme.Accent,
                    BackgroundTransparency = 0.85, -- Subtle glow
                    BorderSizePixel = 0,
                    Size = Library.UDim2(1, 4, 1, 4),
                    Position = Library.UDim2(0, -2, 0, -2),
                    Parent = barBg,
                    ZIndex = barBg.ZIndex - 1,
                })
                
                Library.CreateInstance("UICorner", {
                    CornerRadius = UDim.new(0, IsMobile and 9 or 8),
                    Parent = barGlow,
                })
                ...
                Library.CreateInstance("UICorner", {
                    CornerRadius = UDim.new(0, IsMobile and 7 or 6),
                    Parent = bar,
                })
                
                -- Add draggable knob for better visibility
                local knob = Library.CreateInstance("Frame", {
                    Name = "Knob",
                    BackgroundColor3 = CurrentTheme.Text,
                    BackgroundTransparency = 0,
                    ...
                    Position = Library.UDim2(0, 0, 0.5, 0),
                    AnchorPoint = Vector2.new(0, 0.5),
                    Size = Library.UDim2(0, IsMobile and 22 or 18, 0, IsMobile and 22 or 18), -- Bigger knob
                    Parent = bar,
                    ZIndex = bar.ZIndex + 1,
                })
                ...
                -- Inner glow on knob
                local knobGlow = Library.CreateInstance("Frame", {
                    Name = "KnobGlow",
                    BackgroundColor3 = CurrentTheme.Text,
                    BackgroundTransparency = 0.3,
                    BorderSizePixel = 0,
                    Size = Library.UDim2(1, 2, 1, 2),
                    Position = Library.UDim2(0, -1, 0, -1),
                    Parent = knob,
                ...
                function Slider:UpdateTheme(theme)
                    ...
                    barGlow.BackgroundColor3 = theme.Accent
                    knob.BackgroundColor3 = theme.Text
                    knobGlow.BackgroundColor3 = theme.Text
                end
                ...
                local function SetValue(value)
                    ...
                    local knobPosition = percent * barSize
                    
                    bar:TweenSize(UDim2.new(percent, 0, 1, 0), "Out", "Quad", 0.1, true)
                    knob:TweenPosition(UDim2.new(0, knobPosition, 0.5, 0), "Out", "Quad", 0.1, true)
                ...
                barBg.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                        ...
                        -- Visual feedback
                        bar.BackgroundColor3 = CurrentTheme.AccentHover
                        knob.Size = Library.UDim2(0, IsMobile and 26 or 22, 0, IsMobile and 26 or 22)
                    end
                end)
                ...
                UserInputService.InputEnded:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                        ...
                        knob.Size = Library.UDim2(0, IsMobile and 22 or 18, 0, IsMobile and 22 or 18)
                    end
                end)
